...
class MenuView:
    def __init__(self, master: tk.Widget, on_start: Callable[[], None]) -> None:
        ...
        self.choice_of_texture_button.grid(
            row=4, column=0, padx=12, pady=(8, 16))

        # --- Map choice combobox ---
        self.choice_of_map_combobox = ttk.Combobox(
            self.frame,
            values=["Map 1", "Map 2", "Map 3", "Map 4", "Map 5"],
            state="readonly",
            width=16,
        )

        self.choice_of_map_combobox.grid(
            row=5, column=0, padx=12, pady=(8, 16))
        self.choice_of_map_combobox.set("Map 1")
        self.choice_of_map_combobox.bind(
            "<<ComboboxSelected>>", self.change_map)
        ...
        self.choice_of_car_combobox.grid(
            row=6, column=0, padx=12, pady=(8, 16))
        self.choice_of_car_combobox.set(self.selected_car)
        self.choice_of_car_combobox.bind(
            "<<ComboboxSelected>>", self.change_car_img)

        self.car_img = tk.PhotoImage(
            file=textures_dir.joinpath(self.selected_car))
        self.car_img = self.car_img.zoom(3, 3)
        self.car_img_label = tk.Label(self.frame, image=self.car_img)
        self.car_img_label.grid(row=7, column=0, padx=12, pady=(8, 8))

    def change_map(self, event=None) -> None:
        map_selection = self.choice_of_map_combobox.get()
        map_file = map_selection.lower().replace(" ", "_") + ".json"
        self.selected_map = map_file
        self.set_status(f"Selected map: {map_selection}")
...
class SecondWindow:
    def __init__(self, master: tk.Widget, menu) -> None:
        ...
        self.title_label.grid(
            row=0, column=1, columnspan=2, padx=12, pady=(16, 10))
        ...
        self.img7_label.grid(
            row=6, column=1, columnspan=2, padx=4, pady=(4, 4))
